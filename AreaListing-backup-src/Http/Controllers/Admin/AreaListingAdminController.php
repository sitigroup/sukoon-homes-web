...
class AreaListingAdminController extends Controller
{
    public function index(Request $request)
    {
        ...
        $archivedAreas = $archivedAreasQuery->get();
    ...
    public function destroyArea(Area $area)
    {
        if (
            ...
            $area->update(['workflow_status' => 'archived', 'status' => false, 'archived_at' => now()]);
    ...
    {
        $area->update(['workflow_status' => 'active', 'status' => true, 'archived_at' => null]);
    ...
    public function storeSubArea(Request $request)
    {
        ...
        $subArea = SubArea::updateOrCreate(
            ['area_id' => $request->area_id, 'slug' => Str::slug($this->cleanName($request->name))],
    ...
    public function destroySubArea(SubArea $subArea)
    {
        if (
            ...
            $subArea->update(['workflow_status' => 'archived', 'status' => false, 'archived_at' => now()]);
    ...
    {
        if ($subArea->area && $subArea->area->workflow_status === 'archived') {
            $subArea->area->update(['workflow_status' => 'active', 'status' => true, 'archived_at' => null]);
        }

        $subArea->update(['workflow_status' => 'active', 'status' => true, 'archived_at' => null]);
    ...
    public function normalizeAll()
    {
        ...
        DB::transaction(function () use (&$mergedAreas, &$mergedSubAreas) {
            ...
            $groups = Area::all()->groupBy(fn (Area $area) => mb_strtolower(trim($area->city . '|' . $area->state . '|' . $area->name)));
    ...
    public function approveSuggestion(Request $request, int $id)
    {
        ...
        DB::transaction(function () use ($suggestion, $request) {
            if ($suggestion->type === 'area') {
                Area::updateOrCreate(
                    [
                    'city' => $this->cleanName($suggestion->city),
                    'city_name' => $this->cleanName($suggestion->city),
                    'state' => $this->cleanName($suggestion->state),
                        'country' => $this->cleanName($suggestion->country ?: 'India'),
    ...
    public function rejectSuggestion(Request $request, int $id)
    {
        ...
        $this->markSuggestionReviewed($suggestion->id, 'rejected', $request->input('review_note'));
    ...
    private function areaPayload(Request $request): array
    {
        return [
            ...
            'city' => $this->cleanName($request->city),
            'state' => $this->cleanName($request->state),
            'country' => $this->cleanName($request->country),
