...
const EditProperty = ({ params = [] }) => {
    ...
    // Existing title image URL from API (used when blob fetch fails or for preview)
    const [existingTitleImageUrl, setExistingTitleImageUrl] = useState(null);
    ...
    const handleCheckRequiredFields = (currentTab, nextTab) => {
        ...
        for (const tabToValidate of tabsToValidate) {
            ...
            switch (tabToValidate) {
                ...
                case "imagesVideo":
                    if (mediaFormData.titleImages.length === 0 && !existingTitleImageUrl) {
        ...
        try {
            ...
            if (response && response.data && response.data.length > 0) {
                ...
                if (propertyData.title_image) {
                    ...
                    setExistingTitleImageUrl(titleImageURL);
                    try {
            ...
            if (response && !response.error) {
                ...
                router.push(
                    router?.asPath?.includes("/user")
                        ? `/user/listings?tab=properties&lang=${lang}`
                        : `/agent/properties?lang=${lang}`
                );
            } else {
                ...
                {activeTab === "propertyDetails" && (
                    ...
                    {activeTab === "imagesVideo" && (
                        <ImagesVideoTab
                            ...
                            existingTitleImageUrl={existingTitleImageUrl}
