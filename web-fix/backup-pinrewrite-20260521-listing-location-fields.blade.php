        ...
        (function () {
            ...
            function applyAreaSubAreaClientMatch(wrapper, parts, deferSync) {
                ...
                if (subValue) {
                    ...
                    if ($.fn.select2 && subAreaSelect.hasClass('select2-hidden-accessible')) {
                        subAreaSelect.val(null).trigger('change.select2');
                    } else {
                        subAreaSelect.val('');
                    }
            ...
            window.onPinMoveComplete = function (geocodeResult, lat, lng, options) {
                ...
                return $.ajax({
                    ...
                    if (resp && resp.data && applyResolvedLocationFromServer(wrapper, resp.data, true)) {
                        return;
                    }
                    wrapper.find('.area-listing-detected-area').val(parts.detectedArea);
                    wrapper.find('.area-listing-detected-sub-area').val(parts.detectedSubArea);
                ...
                Promise.resolve(bootMap)
                    ...
                    .then(function (position) {
                        ...
                        if (accuracyM > 500) {
                            ...
                            areaListingNotify('{{ __('GPS accuracy is low') }} (' + rounded + 'm). {{ __('Drag the pin to correct position.') }}', 'warning');
                        }
