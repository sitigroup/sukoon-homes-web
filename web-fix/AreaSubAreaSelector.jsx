    ...
    return (
      <div className={`grid gap-3 ${stacked ? "grid-cols-1" : "grid-cols-1 md:grid-cols-2"}`}>
        ...
        <DropdownField
          ...
          placeholder={!canEditSubArea ? "Select area first" : loading.areas ? "Loading..." : "Select Sub Area"}
        ...
        >
          ...
          {filteredSubAreas.length > 0 ? filteredSubAreas.map((subArea) => (
            ...
            <div className="px-3 py-2 text-sm text-gray-500">
              {canEditSubArea ? "No sub areas found — type a name and tap Use below" : "Select area first"}
            </div>
          )}
  ...
  return (
    <div className={"grid " + gridColumns + " gap-3"}>
      ...
      <div>
        ...
        <input className="primaryBackgroundBg leadColor newBorderColor mb-2 w-full rounded-lg border-[1.5px] px-3 py-2 text-sm focus:outline-none" value={subAreaSearch} onChange={(event) => setSubAreaSearch(event.target.value)} placeholder="Search sub area" disabled={!effectiveSelectedArea} />
        <select className={selectClass} value={selectedLocationAddress?.sub_area_id || ""} onChange={(event) => { const subArea = effectiveSelectedArea?.sub_areas?.find((item) => String(item.id) === event.target.value); selectSubArea(subArea); }} disabled={!effectiveSelectedArea}>
