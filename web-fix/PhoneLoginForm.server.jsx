...
const defaultCountry = (process.env.NEXT_PUBLIC_DEFAULT_COUNTRY || 'in').toLowerCase();
const useIndiaSimplePhone = defaultCountry === 'in';

/** National 10-digit mobile only (strips +91 / 91 prefix from paste). */
function cleanIndiaMobile(input) {
...
const PhoneLoginForm = ({
    ...
    const nationalNumber = useIndiaSimplePhone ? cleanIndiaMobile(value?.number) : value?.number;

    const handleIntlPhoneChange = (phone, data) => {
        setValue((prev) => ({
            ...prev,
            number: phone,
    ...
    const handleIndiaPhoneChange = (e) => {
        const national = cleanIndiaMobile(e.target.value);
        setValue((prev) => ({
            ...prev,
            number: national,
            ...
            <div className="flex w-full flex-col justify-center gap-3 p-3 sm:gap-6 sm:p-3 md:p-4">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <div className="mobile-number">
                        {useIndiaSimplePhone ? (
                            <div className="flex h-14 w-full overflow-hidden rounded-lg border newBorderColor">
                                <div
                                    className="primaryBackgroundBg flex shrink-0 items-center justify-center border-e newBorderColor px-3 text-sm font-medium sm:text-base"
                                    aria-hidden="true"
                                >
                                    +{INDIA_DIAL_CODE}
                                </div>
                                <input
                                    ...
                                    className="primaryBackgroundBg h-14 w-full min-w-0 flex-1 border-0 p-2 text-sm outline-none sm:p-[10px] sm:text-base md:text-base"
                                    value={nationalNumber}
                                    onChange={handleIndiaPhoneChange}
                            ...
                            <PhoneInput
                                ...
                                enableSearch={true}
                                value={value.number}
                                onChange={(phone, data) => handleIntlPhoneChange(phone, data)}
