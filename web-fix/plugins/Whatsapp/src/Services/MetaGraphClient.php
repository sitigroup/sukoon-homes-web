...
class MetaGraphClient
{
    ...
    {
        $result = $this->fetchTemplatesResult();

        return ($result['ok'] ?? false) ? ($result['items'] ?? []) : [];
    }

    /**
     * @return array{ok:bool,items?:array<int,array<string,mixed>>,status?:int,message?:string,body?:mixed}
     */
    public function fetchTemplatesResult(): array
    {
        $settings = $this->settings();
        ...
        try {
            ...
            if (! $response->successful()) {
                $body = $response->json();
                $message = (string) (data_get($body, 'error.message') ?: __('whatsapp::whatsapp.connection_failed'));
