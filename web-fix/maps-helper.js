...
(function (global) {
    ...
    function findComponentName(components, typeList) {
        for (var t = 0; t < typeList.length; t++) {
            for (var i = 0; i < components.length; i++) {
                ...
                if (componentHasType(comp.types || [], typeList[t])) {
    ...
    function extractCityNameFromComponents(components) {
        return findComponentName(components, ['locality']);
    }

    function normalizeDetectKey(value) {
        return String(value || '').toLowerCase().replace(/\s+/g, ' ').trim();
    }

    function parseFormattedAddressFirstSegment(formatted, cityName) {
        if (!formatted) {
            return '';
        }
        var parts = formatted.split(',').map(function (part) {
            return part.trim();
        }).filter(Boolean);
        var cityNorm = normalizeDetectKey(cityName);
        for (var i = 0; i < parts.length; i++) {
            var partNorm = normalizeDetectKey(parts[i]);
            if (cityNorm && (partNorm === cityNorm || partNorm.indexOf(cityNorm) >= 0 || cityNorm.indexOf(partNorm) >= 0)) {
                if (i > 0) {
                    var candidate = parts[i - 1];
                    if (candidate && !/^\d+$/.test(candidate.replace(/\s/g, ''))) {
                        return candidate;
                    }
                }
                break;
    ...
    function titleCaseDetected(value) {
        return String(value || '').toLowerCase().replace(/\b\w/g, function (letter) {
            return letter.toUpperCase();
        }).trim();
    }

    function extractDetectedAreaSubArea(geocodeResult) {
        var out = { area: '', subArea: '', areaSource: '', subAreaSource: '' };
        if (!geocodeResult) {
            return out;
        }

        var components = geocodeResult.address_components || [];
        var cityName = extractCityNameFromComponents(components);
        var area = '';
        var areaSource = '';
        var areaTypeOrder = [
        ...
        for (var a = 0; a < areaTypeOrder.length; a++) {
            if (area) {
                break;
            }
            var areaCandidate = findComponentName(components, [areaTypeOrder[a][0]]);
            if (areaCandidate) {
                area = areaCandidate;
                areaSource = areaTypeOrder[a][1];
            }
        }

        if (!area) {
            area = parseFormattedAddressFirstSegment(geocodeResult.formatted_address || '', cityName);
            if (area) {
                areaSource = 'formatted_address';
            }
        }

        var subArea = '';
        var subAreaSource = '';
        if (areaSource === 'sublocality_level_1') {
            subArea = findComponentName(components, ['sublocality_level_2']);
            if (subArea) {
                subAreaSource = 'sublocality_level_2';
            } else {
                var neighborhood = findComponentName(components, ['neighborhood']);
                if (neighborhood && normalizeDetectKey(neighborhood) !== normalizeDetectKey(area)) {
                    subArea = neighborhood;
                    subAreaSource = 'neighborhood';
                }
            }
        }
        ...
        out.area = area;
        out.subArea = subArea;
        out.areaSource = areaSource;
        out.subAreaSource = subAreaSource;
        return out;
    }

    function setDetectedAreaFormFields(areaName, subName) {
        var areaInput = document.getElementById('detected_area_name_input') || document.querySelector('.area-listing-detected-area');
        var subInput = document.getElementById('detected_sub_area_name_input') || document.querySelector('.area-listing-detected-sub-area');
        if (areaInput) {
        ...
        if (typeof global.jQuery !== 'undefined') {
            global.jQuery(document).trigger('area-listing-detected-updated');
        }
    }

    function applyDetectedAreaFromResolve(geocodeResult, resolveData) {
        var resolved = resolveData || {};
        if (resolved.area_id) {
            global.__detectedAreaName = '';
            global.__detectedAreaSource = '';
            setDetectedAreaFormFields('', global.__detectedSubAreaName || '');
            var subInput = document.getElementById('detected_sub_area_name_input') || document.querySelector('.area-listing-detected-sub-area');
            if (subInput) {
                subInput.value = global.__detectedSubAreaName || subInput.value || '';
            }
            return { cleared: true, areaMatched: true };
        }

        var extracted = extractDetectedAreaSubArea(geocodeResult);
        var areaName = titleCaseDetected(extracted.area);
        var subName = titleCaseDetected(extracted.subArea);
        global.__detectedAreaName = areaName;
        global.__detectedSubAreaName = subName;
        global.__detectedAreaSource = extracted.areaSource || '';
        global.__detectedSubAreaSource = extracted.subAreaSource || '';
        setDetectedAreaFormFields(areaName, subName);
        return extracted;
    }

    global.extractDetectedAreaSubArea = extractDetectedAreaSubArea;
    global.applyDetectedAreaFromResolve = applyDetectedAreaFromResolve;
    global.setDetectedAreaFormFields = setDetectedAreaFormFields;
