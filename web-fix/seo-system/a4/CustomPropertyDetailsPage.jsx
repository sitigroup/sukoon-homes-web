...
import usePlatformCapabilities from "@/plugins/platform-capabilities/usePlatformCapabilities";
...
/** Public listings: agent contact only — never expose owner name/phone. */
const getPublicAgentContact = (property, { ownerMode = false } = {}) => {
  if (ownerMode) {
    return {
...
const CustomPropertyDetailsPage = ({
  OfficialFallback,
  ownerMode = false,
  initialPropertyLoad = null,
}) => {
  const { can: platformCan, loading: platformCapsLoading } = usePlatformCapabilities({ asRole: "user" });
  const router = useRouter();
  const { slug, lang } = router.query;
  const [property, setProperty] = useState(initialPropertyLoad?.property ?? null);
  ...
  const agentContact = getPublicAgentContact(property, { ownerMode });
  const agentName = ownerMode
    ? agentContact.name || "Listing Owner"
    : agentContact.isAgent
      ? agentContact.name || t("pdAgentContactLabel")
      : "";
  const agentMobile = normalizePhone(agentContact.mobile);
  const canCallAgent = Boolean(agentMobile);
  const whatsappUrl = canCallAgent ? `https://wa.me/${agentMobile.replace(/[^\d]/g, "")}` : "";
  ...
  const handleSendEnquiry = async () => {
    ...
    if (!platformCan("send_enquiry")) return;
  ...
  const canSendEnquiry =
    platformCapsLoading || platformCan("send_enquiry");
  const canScheduleVisit =
    (platformCapsLoading || platformCan("schedule_visit")) &&
  ...
  const tabItems = [
    ...
    ...(layoutSettings.showNearby && nearbyFacilities.length > 0 ? [{ id: "nearby", label: "Nearby Places" }] : []),
  ...
  const primaryContactActions = (
    <div className="grid gap-1.5">
      {canCallAgent && (
        <ActionButton href={`tel:${agentMobile}`} icon={FiPhone} variant="green">
          Call Now
        </ActionButton>
      )}
      {canCallAgent && (
      ...
      {canSendEnquiry ? (
        <ActionButton onClick={handleSendEnquiry} icon={FiMessageCircle} variant="bronze">
          {isInterested ? t("pdEnquirySent") : t("pdSendEnquiry")}
        </ActionButton>
      ) : null}
      {!canCallAgent && !canScheduleVisit && (
        <p className="rounded-md bg-slate-50 p-2.5 text-xs leading-5 text-slate-600">
          {t("pdContactUnavailable")}
        </p>
      )}
    </div>
  );
  ...
  const locationContent = (
    <ContentCard>
      <SectionTitle>{t("pdPublicLocation")}</SectionTitle>
      <div className="rounded-lg bg-emerald-50 p-3">
        <div className="flex gap-2.5 text-sm leading-snug text-slate-800">
          ...
          <div>
            <div className="font-semibold">{publicLocation || t("pdLocationOnRequest")}</div>
            <p className="mt-1.5 leading-relaxed text-slate-600">
              {t("pdLocationPrivacyNote")}
            </p>
          </div>
        </div>
      </div>
  ...
  const tabContent = {
    ...
    ...(nearbyContent ? { nearby: nearbyContent } : {}),
        ...
        <ContentCard>
          ...
          <div className="flex items-center gap-2.5">
            <div className="relative h-12 w-12 overflow-hidden rounded-full bg-slate-100">
              <ImageWithPlaceholder
                src={agentContact.image || placeholder}
                alt={agentName || t("pdAgentContactLabel")}
                width={96}
                height={96}
            ...
            <div className="min-w-0">
              {agentName ? (
                <div className="truncate text-sm font-semibold text-slate-950">{agentName}</div>
              ) : agentContact.isAgent ? (
                <div className="truncate text-sm font-semibold text-slate-950">{t("pdAgentContactLabel")}</div>
              ) : null}
    ...
    <section className="overflow-x-hidden bg-[#f5f5f1] pb-20 text-[15px] text-slate-950 lg:pb-8">
      ...
      {!ownerMode && (
        <div className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 px-4 py-2 shadow-[0_-8px_24px_rgba(15,23,42,0.1)] backdrop-blur lg:hidden">
          <div className={`mx-auto grid max-w-[1080px] gap-1.5 ${canCallAgent ? "grid-cols-3" : "grid-cols-1"}`}>
            {canCallAgent && (
              <ActionButton href={`tel:${agentMobile}`} icon={FiPhone} variant="green" className="!min-h-[38px] !px-2 text-xs">
                Call
              </ActionButton>
            )}
            {canCallAgent && (
