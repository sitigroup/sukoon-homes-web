...
const Layout = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const onEnd = () => setIsRouteChanging(false);
  ...
  useEffect(() => {
    ...
    // Allow agents on personal self-verification (₹49 KYC) while in agent role
    const pathname = router?.pathname || "";
    const isAgentDetailRoute =
      pathname.startsWith("/agent/my-property") ||
      pathname.startsWith("/agent/my-project");
    const isAgentSelfKycRoute =
      pathname === "/my-profile/verification" ||
      pathname === "/my-profile/verification/payment-callback" ||
      pathname.startsWith("/my-profile/verification/");

    if (currentRole === "agent" && !isAgentDetailRoute && !isAgentSelfKycRoute) {
      toast.error(t("pleaseSwitchToUser"), { id: "agent-switch-user" });
  ...
  useEffect(() => {
    const handleLanguageFromUrl = async () => {
      ...
      if (
        router.pathname?.startsWith("/property-detail-preview") ||
        (router.asPath || "").split("?")[0].startsWith("/property-detail-preview")
      ) {
        return;
      }
      ...
      if (router.pathname === "/" && asPathBase !== "/") {
        return;
      }
      if (router.pathname !== "/" && asPathBase === "/") {
        return;
      }
      if (router.pathname.includes("[") && !router.query.slug) {
        return;
      }

      const langFromAsPath = asPathParams.get("lang") || "";
      const lang = typeof router.query.lang === "string" ? router.query.lang : langFromAsPath;
      ...
      try {
        const [, queryPart = ""] = router.asPath.split("?");
        const params = new URLSearchParams(queryPart);
        params.set("lang", langToUse);
        router.replace(`${asPathBase}?${params.toString()}`, undefined, { shallow: true });
      } catch (error) {
