...
export const handlePackageCheck = async (
  ...
  // Premium projects: package check then public project details (agent-owned listings are viewable here)
  if (type === PackageTypes.PREMIUM_PROJECTS) {
    ...
    return router.push(`/project-details/${id}?lang=${locale}`);
  }
...
export const getDefaultSchemaMarkup = () => {
  ...
  const orgName =
    ...
    "Sukoon Homes";
  const logoUrl =
    process.env.NEXT_PUBLIC_WEB_LOGO || `${webUrl}/favicon.ico`;
  ...
  const knowsAbout = (
    process.env.NEXT_PUBLIC_SCHEMA_KNOWS_ABOUT ||
    "Rental properties,Property verification,Tenant KYC,Online rent agreement"
  )
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean);
  ...
  const areaNames = (
    process.env.NEXT_PUBLIC_SCHEMA_AREAS_SERVED ||
    "Barmer,Jodhpur,Rajasthan,India"
  )
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean);
  ...
  const areaServed = areaNames.map((name) => {
    if (name === "India" || name === "Rajasthan") {
      return { "@type": "AdministrativeArea", name };
    }
    return {
      "@type": "City",
      name,
      containedInPlace: { "@type": "AdministrativeArea", name: "Rajasthan" },
    };
  });
    ...
    "@graph": [
      {
        ...
        potentialAction: {
          ...
          target: {
            "@type": "EntryPoint",
            urlTemplate: `${webUrl}/search/{search_term_string}`,
          },
      ...
      },
      {
        ...
        "@id": `${webUrl}/#realestateagent`,
        name: orgName,
        url: webUrl,
        image: logoUrl,
        areaServed,
        parentOrganization: { "@id": `${webUrl}/#organization` },
      },
    ],
  };
};
