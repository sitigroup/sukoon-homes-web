...
class AreaListingService
{
  ...
  /** Area suggestion row id created/updated in the current saveListingLocation request (for pairing). */
    private static ?int $lastAreaSuggestionRowIdThisRequest = null;

    public static function resetListingSuggestionPairing(): void
    {
        self::$lastAreaSuggestionRowIdThisRequest = null;
    }
    ...
    public static function storePendingSuggestion(string $type, array $data): void
    {
        ...
        $groupToken = ! empty($data['group_token']) ? (string) $data['group_token'] : null;
        if ($type === 'sub_area' && ! empty($data['pair_with_area_suggestion']) && self::$lastAreaSuggestionRowIdThisRequest) {
            $groupToken = $groupToken ?: (string) Str::uuid();
            DB::table('area_listing_suggestions')
                ->where('id', self::$lastAreaSuggestionRowIdThisRequest)
        ...
        if ($pendingExists) {
            ...
            if ($groupToken) {
                $updatePayload['group_token'] = $groupToken;
            }
            ...
            if ($type === 'area') {
                self::$lastAreaSuggestionRowIdThisRequest = (int) DB::table('area_listing_suggestions')
                    ->where('type', 'area')
                    ->where('status', 'pending')
                    ->where('normalized_name', $normalized)
                    ->value('id');
        ...
        $rowId = (int) DB::table('area_listing_suggestions')->insertGetId($insert);
        if ($type === 'area') {
            self::$lastAreaSuggestionRowIdThisRequest = $rowId;
        }
    }
    ...
    {
        self::resetListingSuggestionPairing();
        ...
        if (empty($subAreaId) && $detectedSubArea !== '') {
            ...
            if (! $targetAreaId && $detectedArea !== '') {
                $cityIdForSub = $city?->id ?: (self::pickScalar($request->input('city_id')) ? (int) $request->input('city_id') : null);
                if ($cityIdForSub) {
