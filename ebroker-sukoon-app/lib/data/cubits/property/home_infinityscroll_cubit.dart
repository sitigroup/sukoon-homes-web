...
class HomePageInfinityScrollCubit extends Cubit<HomePageInfinityScrollState> {
  ...
  bool hasMoreData() {
    if (state is HomePageInfinityScrollSuccess) {
      return (state as HomePageInfinityScrollSuccess).properties
  ...
  Future<void> fetchMore() async {
    if (state is HomePageInfinityScrollSuccess) {
      try {
        ...
        emit(
          (state as HomePageInfinityScrollSuccess).copyWith(
            isLoadingMore: true,
          ),
        );

        final dataOutput = await propertyRepository.fetchAllProperties(
          offset: (state as HomePageInfinityScrollSuccess).properties
              .whereType<PropertyModel>()
              .length,
          latitude: HiveUtils.getLatitude().toString(),
          longitude: HiveUtils.getLongitude().toString(),
