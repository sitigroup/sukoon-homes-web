...
function areaListingRepairRenderDryRun(data) {
    ...
    if (missing === 0) {
        html = '<div class="alert alert-success mb-0">{{ __('All property location rows are present. No repair needed.') }}</div>';
        $executePanel.addClass('d-none');
    } else {
        html += '<div class="alert alert-warning mb-3">' + missing + ' {{ __('properties found with missing location rows.') }}</div>';
        if (Array.isArray(data.sample) && data.sample.length) {
            html += '<div class="table-responsive"><table class="table table-sm table-bordered mb-0"><thead><tr><th>{{ __('ID') }}</th><th>{{ __('Title') }}</th></tr></thead><tbody>';
            data.sample.forEach((item) => {
                html += '<tr><td>' + areaListingRepairEscape(item.id) + '</td><td>' + areaListingRepairEscape(item.title) + '</td></tr>';
            });
...
$('#areaListingRepairDryRunBtn').on('click', function () {
    ...
    areaListingRepairPost(areaListingRepairDryRunUrl)
        ...
        .catch((error) => {
            $('#areaListingRepairDryRunResult').removeClass('d-none').html(
                '<div class="alert alert-danger mb-0">' + areaListingRepairEscape(error.message || 'Dry run failed') + '</div>'
            );
        })
...
function areaListingDriftRenderCheck(data) {
    ...
    if (total === 0) {
        html = '<div class="alert alert-success mb-0">{{ __('No drift found. All snapshots match.') }}</div>';
        ...
        html += '<div class="alert alert-warning mb-3">' + snapshot + ' {{ __('areas have drifted snapshots.') }} ' + missing + ' {{ __('areas missing city_id.') }}</div>';
        html += areaListingDriftRenderTable(data.sample, '{{ __('Sample') }}');
        $('#areaListingDriftPreviewPanel').toggleClass('d-none', snapshot === 0);
        $('#areaListingDriftConfirmPanel').addClass('d-none');
        $('#areaListingDriftPreviewResult').addClass('d-none').empty();
    }

    $('#areaListingDriftCheckResult').removeClass('d-none').html(html);
}
...
$('#areaListingDriftCheckBtn').on('click', function () {
    ...
    areaListingDriftPost(areaListingDriftCheckUrl)
        ...
        .catch((error) => {
            $('#areaListingDriftCheckResult').removeClass('d-none').html(
                '<div class="alert alert-danger mb-0">' + areaListingRepairEscape(error.message || 'Drift check failed') + '</div>'
            );
        })
        .finally(() => $btn.prop('disabled', false));
});
...
$('#areaListingDriftPreviewBtn').on('click', function () {
    ...
    areaListingDriftPost(areaListingDriftPreviewUrl)
        .then(({ ok, body }) => {
            ...
            let html = '<div class="alert alert-info mb-3">{{ __('Preview only — no database changes were made.') }} ' + wouldUpdate + ' {{ __('snapshots would be updated.') }}</div>';
            html += areaListingDriftRenderTable(body.sample, '{{ __('Planned updates') }}');
            $('#areaListingDriftPreviewResult').removeClass('d-none').html(html);
            $('#areaListingDriftConfirmPanel').toggleClass('d-none', wouldUpdate === 0);
        })
        .catch((error) => {
            $('#areaListingDriftPreviewResult').removeClass('d-none').html(
                '<div class="alert alert-danger mb-0">' + areaListingRepairEscape(error.message || 'Preview failed') + '</div>'
            );
        })
        .finally(() => $btn.prop('disabled', false));
});
...
$('#areaListingDriftConfirmBtn').on('click', function () {
    ...
    areaListingDriftPost(areaListingDriftExecuteUrl)
        .then(({ ok, body }) => {
            ...
            $('#areaListingDriftSyncResult').removeClass('d-none').html(
                '<div class="alert alert-info mb-0">{{ __('Sync complete.') }} ' + updated + ' {{ __('snapshots updated.') }}</div>'
            );
            $('#areaListingDriftConfirmPanel').addClass('d-none');
            $('#areaListingDriftPreviewPanel').addClass('d-none');
        ...
        .catch((error) => {
            $('#areaListingDriftSyncResult').removeClass('d-none').html(
                '<div class="alert alert-danger mb-0">' + areaListingRepairEscape(error.message || 'Sync failed') + '</div>'
            );
        })
        .finally(() => $btn.prop('disabled', false));
});
...
$('#areaListingRepairExecuteBtn').on('click', function () {
    ...
    areaListingRepairPost(areaListingRepairExecuteUrl)
        .then(({ ok, body }) => {
            ...
            let html = '<div class="alert alert-info mb-0">{{ __('Repair complete.') }} ' + created + ' {{ __('rows created.') }} ' + errorCount + ' {{ __('errors.') }}</div>';
            if (Array.isArray(body.errors) && body.errors.length) {
                html += '<ul class="mt-2 mb-0">';
                body.errors.forEach((item) => {
                    html += '<li>#' + areaListingRepairEscape(item.property_id) + ': ' + areaListingRepairEscape(item.message) + '</li>';
                });
                html += '</ul>';
            }

            $('#areaListingRepairExecuteResult').removeClass('d-none').html(html);
            $('#areaListingRepairExecutePanel').addClass('d-none');
        ...
        .catch((error) => {
            $('#areaListingRepairExecuteResult').removeClass('d-none').html(
                '<div class="alert alert-danger mb-0">' + areaListingRepairEscape(error.message || 'Repair failed') + '</div>'
            );
        })
        .finally(() => $btn.prop('disabled', false));
});
