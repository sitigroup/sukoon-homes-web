        ...
        (function () {
            function bootAreaListingNativeSync() {
                ...
                const syncOne = function (root) {
                    ...
                    Array.from(areaSelect.options).forEach(function (option) {
                        ...
                        option.disabled = !hasCity || (!!optionCityId && optionCityId !== cityId);
                    });
            ...
            function syncSelects(wrapper) {
                ...
                wrapper.find('.area-listing-admin-area option').each(function () {
                    ...
                    option.prop('disabled', !cityId || (!!optionCityId && optionCityId !== cityId));
                });
                if (!cityId || wrapper.find('.area-listing-admin-area option:selected').is(':disabled')) {
                ...
                wrapper.find('.area-listing-admin-area').prop('disabled', !cityId);
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    navigator.geolocation.getCurrentPosition(function (position) {
                        ...
                        watchId = navigator.geolocation.watchPosition(function (watched) {
                            pickBetter(watched);
                            if (bestPosition && bestPosition.coords && (!bestPosition.coords.accuracy || bestPosition.coords.accuracy <= 3000)) {
                                clearTimeout(timer);
                                finish();
                            }
            ...
            function fillAreaListingFromPlaceResult(wrapper, result, lat, lng) {
                if (result) {
                    ...
                    syncSelects(wrapper);
                    const areaMatched = selectOptionByText(wrapper.find('.area-listing-admin-area'), function (option) {
                        const label = titleCase(option.text().split(' - ').pop());
                        return detectedArea && label === detectedArea;
                    });
                    syncSelects(wrapper);
                    const subMatched = selectOptionByText(wrapper.find('.area-listing-admin-sub-area'), function (option) {
                        const label = titleCase(option.text().split(' - ').pop());
                ...
                [200, 800, 1600].forEach(function (delay) {
                    setTimeout(function () { syncSelects(wrapper); }, delay);
                });
            }
            ...
            function reverseGeocodeForAreaListing(wrapper, lat, lng) {
                ...
                if (window.applyBackendPlaceFromCoords) {
                    return new Promise(function (resolve, reject) {
                        const request = window.applyBackendPlaceFromCoords(lat, lng, mapSelectors, function (result, resolvedCoords) {
                            try {
                        ...
                        if (request && typeof request.fail === 'function') {
                            request.fail(function () {
                                reject(new Error('{{ __('Unable to resolve address from coordinates.') }}'));
                            });
                        }
                    });
                }
            ...
            $(document).on('click', '.area-listing-current-location', function (event) {
                ...
                requestAreaListingPosition()
                    .then(function (position) {
                        ...
                        if (window.moveBackendPlacesMapPin) {
                            window.moveBackendPlacesMapPin(lat, lng, mapSelectors);
                        }

                        const accuracyM = Number(position.coords.accuracy || 0);
                        if (accuracyM > 3000) {
                            const km = Math.max(1, Math.round(accuracyM / 1000));
                            areaListingNotify('{{ __('GPS accuracy is about') }} ' + km + ' {{ __('km. City name may be wrong on desktop — drag the map pin to your exact spot.') }}', 'warning');
                        }
            ...
            $(document).on('submit', 'form', function () {
                ...
                form.find('.area-listing-admin-area, .area-listing-admin-sub-area').each(function () {
