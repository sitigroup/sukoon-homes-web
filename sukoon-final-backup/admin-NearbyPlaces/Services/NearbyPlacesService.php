...
class NearbyPlacesService
{
    ...
    public function getForProperty(int $propertyId, bool $forceRefresh = false, bool $adminPreview = false, ?int $categoryId = null): array
    {
        ...
        $publicDirectionsEnabled = NearbyPlacesSettings::publicDirectionsEnabled();
        if (!$adminPreview && !$publicDirectionsEnabled) {
            $places = $this->stripDirectionsFromPlaces($places);
        }
        ...
        $payload = [
            ...
            'public_directions_enabled' => $publicDirectionsEnabled,
    ...
    /**
     * Remove directions URLs from public responses so customers cannot infer property coordinates.
     *
     * @param  array<string, array<int, array<string, mixed>>>  $places
     * @return array<string, array<int, array<string, mixed>>>
     */
    protected function stripDirectionsFromPlaces(array $places): array
    {
        foreach ($places as $slug => $items) {
            foreach ($items as $i => $item) {
                $places[$slug][$i]['show_directions'] = false;
                $places[$slug][$i]['directions_url'] = '';
                $places[$slug][$i]['invalid_navigation'] = false;
                $places[$slug][$i]['navigation_flags'] = [];
            }
        }
